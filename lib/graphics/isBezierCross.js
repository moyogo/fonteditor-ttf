    ...
    if (isBoundingBoxCross(bounding1, bounding2)) {
        ...
        if (tResult) {
            ...
            for (var i = tResult.length - 1; i >= 0; i--) {
                tr1 = tResult[i];
                t2Result = bezierQ2Equation(ymatrix[3] - xmatrix[3], ymatrix[4] - xmatrix[4], xmatrix[0] * Math.pow(tr1, 2) + xmatrix[1] * tr1 + xmatrix[2] - (ymatrix[0] * Math.pow(tr1, 2) + ymatrix[1] * tr1 + ymatrix[2]));
                if (!t2Result) {
                    tResult.splice(i, 1);
                } else {
            ...
            return tResult.length ? tResult.map(function (t) {
                return ceilPoint({
